...
public class LeboncoinService {
    ...
    public void putProductsOnline(PutOnlineTask task) {
...
//                            List<WebElement> needInputs = allInputs.stream().filter(webElement -> webElement.getText().contains("*")).toList();
                                ...
                                for (WebElement needInput : needInputs) {
                                    try {
                                        ...
                                        String elementName = inputElement.getAttribute("name");
                                        if (StrUtil.equals(Optional.ofNullable(inputElement.getAttribute("aria-hidden")).orElse(""), "true")) {
                                            log.info("{} aria-hidden,无需点击", elementName);
                                            continue;
                                        }
                                    ...
                                    sleep(1);
                                    List<WebElement> li;
                                    li = needInput.findElements(By.tagName("li"));
                                    ...
                                    if (!li.isEmpty()) {
                                        ...
                                        AiSelectOptionRes data = Optional.ofNullable(aiSelectOptionResResponseResult.getData())
                                                .orElseGet(AiSelectOptionRes::new);
                                        log.info("选择:{}", JSONUtil.toJsonStr(data));
                                        String selectedValue = Optional.ofNullable(data.getSelectedValue())
                                                .orElseGet(() -> needInput.findElement(By.tagName("input")).getAttribute("value"));
...
//                                            sleep(1);
                                            ...
                                            case "location":
                                                ...
                                                handleAddressSelection(driver, element, addrKeyword);
                                                break;
...
//                                ).get(0).click();
                                Optional<WebElement> change = driver.findElements(
                                        By.xpath("//form//button")
                                ).stream().filter(webElement -> StrUtil.equalsAnyIgnoreCase(webElement.getText(), "Modifier les modes de livraison", "Modifier les transporteurs")).findFirst();
                                if (change.isEmpty()) {
                                    throw new RuntimeException("未找到修改物流按钮");
                                }
                                change.get().click();
...
//                            List<WebElement> needInputs = allInputs.stream().filter(webElement -> webElement.getText().contains("*")).toList();
                                ...
                                for (WebElement needInput : needInputs) {
                                    ...
                                    if (!li.isEmpty()) {
                                        ...
                                        log.info("选择:{}", JSONUtil.toJsonStr(data));
                                        String selectedValue = data == null ? needInput.findElement(By.tagName("input")).getAttribute("value") : data.getSelectedValue();
                                        Optional<WebElement> any = li.stream().filter(webElement -> StrUtil.equalsIgnoreCase(webElement.getText(), selectedValue)).findAny();
                                        if (any.isPresent()) {
                                            any.get().click();
                                        } else {
                                                ...
                                                if (!orignLi.isEmpty()) {
                                                    ...
                                                    while (true) {
                                                        try {
                                                            ...
                                                            log.info("地址长度:{}", addrInputLi.size());
                                                            sleep(1);
                                                            ...
                                                            if (any.isPresent()) {
                                                                any.get().click();
                                                                break;
                                                            } else if (!addrInputLi.isEmpty()) {
                                                                log.info("地址选择项不合适，重试");
                                                                continue;
                                                            } else {
                                                                throw new RuntimeException("未找到指定地址:" + addrKeyword);
                                                            }
                                                        } catch (StaleElementReferenceException e) {
                                                            log.info("地址输入框元素已失效，重新获取");
                                                        }
                                                    }
                                                }
                                                break;
...
// 存储截图
    ...
    private void handleAddressSelection(WebDriver driver, WebElement addrInput, String addrKeyword) {
        addrInput.click();
        List<WebElement> orignLi = untilFindsOrEmpty(driver, By.tagName("li"), 10);
        if (!orignLi.isEmpty()) {
            orignLi.get(0).click();
        ...
        while (retryCount++ < 3) {
            try {
                List<WebElement> addrInputLi = untilPresences(driver, By.tagName("li"), 30);
                Optional<WebElement> validOption = addrInputLi.stream()
                        .filter(l -> "option".equals(l.getAttribute("role")))
                        .filter(l -> "false".equals(l.getAttribute("aria-disabled")))
                        .findFirst();

                if (validOption.isPresent()) {
                    validOption.get().click();
                    return;
                }
                new WebDriverWait(driver, Duration.ofSeconds(1))
                        .until(ExpectedConditions.elementToBeClickable(validOption.get()));
            } catch (StaleElementReferenceException e) {
                log.info("地址输入框元素已失效，重试次数：{}/3", retryCount);
            }
        }
        throw new RuntimeException("未找到指定地址:" + addrKeyword);
    }
